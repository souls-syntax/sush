...
size_t redirectionIdentifier(std::vector<Token> &raw_segments,std::vector<IOData> &redirections) {
    
    int i = 0;
    while (i < raw_segments.size()) { 
        if (raw_segments[i].parts.size() == 1 && raw_segments[i].parts[0].quote == QuoteType::None) {
            std::string sm = raw_segments[i].parts[0].text;
            bool is_operator = true;
            RedirectType rt;
            int fd = -1;

            if (sm == ">" || sm == "1>")       { rt = RedirectType::Output;  fd = 1; }
            else if (sm == ">>" || sm == "1>>") { rt = RedirectType::Append;  fd = 1; }
            else if (sm == "<")  { rt = RedirectType::Input;   fd = 0; }
            else if (sm == "<<") { rt = RedirectType::Heredoc; fd = 0; }
            else if (sm == "2>") { rt = RedirectType::Output;  fd = 2; }
            else if (sm == "2>>") { rt = RedirectType::Append;  fd = 2; }
